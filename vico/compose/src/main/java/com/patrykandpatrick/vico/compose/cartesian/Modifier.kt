 ...
 */
private fun createSnapBehavior(
  scrollState: VicoScrollState,
  config: SnapBehaviorConfig? = null,
): FlingBehavior {
  ...
  val snapLayoutInfoProvider = object : SnapLayoutInfoProvider {
    ...
    override fun calculateApproachOffset(velocity: Float, decayOffset: Float): Float {
      ...
      val isNewFling =
        kotlin.math.abs(velocity - lastVelocity) > snapConfig.velocityThresholds.velocityChangeThreshold ||
          (currentTime - lastApproachTime) > snapConfig.velocityThresholds.timeGapThreshold

      if (isNewFling) {
        Log.i(
          "SnapTargetFunction",
          "New fling detected - velocity: $velocity, lastVelocity: $lastVelocity",
        )
      ...
      Log.i(
        "SnapBehavior",
        "Approach Phase - windowWidthPx: $windowWidthPx, windowsToMove: $windowsToMove, velocity: $velocity",
      )
    ...
    override fun calculateSnapOffset(velocity: Float): Float {
      ...
      val approachedLabel =
        snapConfig.snapToLabelFunction?.let { it(initialRange, isDrag, velocity.sign > 0f) }
      ...
      val finalSnapOffset =
        if (approachedLabel != null && layerDimensions != null && context != null && bounds != null) {
          // Same convention as GraphView initial scroll: x = viewport start (first data point), no padding subtraction.
          // Scroll.Absolute.x(x) puts x at the viewport left edge, so use approachedLabel directly.
          ...
          Log.i(
            "SnapTargetFunction",
            "Snap Phase - approachedLabel: $approachedLabel, targetScrollPosition: $targetScrollPosition, currentScrollPosition: $currentScrollPosition, offset: $offset",
          )
