...
internal class MonotonePointConnector : LineCartesianLayer.PointConnector {
  override fun connect(
    ...
    val s0 =
      if (dx0 != 0.0) (neighbors[3].y - neighbors[2].y) / dx0 else 0.0  // entry2.y - entry1.y
    ...
    if (isLastSegment) {
      ...
      if (y1 > y2) {
        ...
        endpointMarginApplied =
          "lastSegment: c2y=$c2yBeforeEndpointMargin->$c2y (preserve original from extrapolation)"
      } else if (y1 < y2) {
    ...
    Log.d(
      "MonotonePointConnector",
      "=== DEBUG Bézier Monotonicity ===\n" +
        ...
        "KEY_SAMPLES: ${
          keySamplePoints.zip(keySampleY)
            .joinToString(", ") { "t=${it.first}=${"%.2f".format(it.second)}" }
        }\n" +
    ...
    // Note: y2 cannot be changed (it's the actual data point), but c2y has been adjusted for endpoint margin
    Log.d(
      "MonotonePointConnector",
      "PATH_CUBIC_TO: c1x=$c1x c1y=$c1y c2x=$c2x c2y=$c2y x2=$x2 y2=$y2\n" +
        "DATA_VALUES: entry1.y=${entry1.y} entry2.y=${entry2.y}\n" +
        "SCREEN_COORDS: x1=$x1 y1=$y1 x2=$x2 y2=$y2",
    )
